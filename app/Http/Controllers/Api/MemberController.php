...
use App\Models\Entity\TempEmail;
use App\Models\Entity\TempPhone;
...
class MemberController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if ($email == '' && $phone == '') {
            $m3_result->status  = 1;
            $m3_result->msg = '手机号或邮箱不能为空';
            return $m3_result->toJson();
        }
        if ($password == '' || strlen($password) < 6) {
            $m3_result->status  = 2;
            $m3_result->msg = '密码不少于6位';
            return $m3_result->toJson();
        }
        if ($confirm == '' || strlen($confirm) < 6) {
            $m3_result->status  = 3;
            $m3_result->msg = '确认密码不少于6位';
            return $m3_result->toJson();
        }
        if ($password != $confirm) {
            $m3_result->status  = 4;
            $m3_result->msg = '两次密码不相同';
            return $m3_result->toJson();
        }
